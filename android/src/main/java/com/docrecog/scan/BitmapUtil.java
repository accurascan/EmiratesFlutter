...
public class BitmapUtil {
    ...
    public static Bitmap crop(final Bitmap src, final int w, final int h,
        ...
        final float scale = Math.max(
                (float) w / srcWidth,
                (float) h / srcHeight);
        Log.e("crop", "scale: " + scale);
        m.setScale(scale, scale);
        final int srcCroppedW, srcCroppedH;
        int srcX, srcY;
        srcCroppedW = Math.round(w / scale);
        srcCroppedH = Math.round(h / scale);
//        Log.e("crop", "(srcCroppedW, srcCroppedH): (" + srcCroppedW + "," + srcCroppedH + ")");
//        Log.e("crop", "(srcWidth, srcHeight): (" + srcWidth + "," + srcHeight + ")");
        srcX = (int) (srcWidth * horizontalCenterPercent - srcCroppedW / 2); // 0
        srcY = (int) (srcHeight * verticalCenterPercent - srcCroppedH / 2);  // 320
//        Log.e("crop", "crop: y " + srcY);
        // Nudge srcX and srcY to be within the bounds of src
        srcX = Math.max(Math.min(srcX, srcWidth - srcCroppedW), 0);  // 0
        srcY = Math.max(Math.min(srcY, srcHeight - srcCroppedH), 0); // 320
//        Log.e("crop", "crop: y  " + srcY);
