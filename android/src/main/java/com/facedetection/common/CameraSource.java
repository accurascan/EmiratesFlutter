...
public class CameraSource {
    ...
    public static final int DEFAULT_REQUESTED_CAMERA_PREVIEW_WIDTH = 1280; ///*480;*/1280;
    public static final int DEFAULT_REQUESTED_CAMERA_PREVIEW_HEIGHT = 1280; ///*360;*/960;
    ...
    public synchronized void stop() {
        ...
        if (processingThread != null) {
            try {
                ...
                AccuraFaceMatchLog.loge(TAG, "Processing interrupted.");
            }
        ...
        if (camera != null) {
            ...
            try {
                ...
                AccuraFaceMatchLog.loge(TAG, "Failed to clear camera preview: \n" + e.toString());
            }
            camera.release();
            camera = null;
        }
...
//        }
        ...
        Camera.Size tempPreviewSize = parameters.getPreviewSize();
        Camera.Size tempPictureSize = parameters.getPictureSize();
//        previewSize = new Size(tempPreviewSize.width , tempPreviewSize.height);
//        Logger.e(TAG, "Default Preview size (w,h) (" + tempPreviewSize.width + "," + tempPreviewSize.height +
//         ")(" + sizePair.preview.getWidth() + "," + sizePair.preview.getHeight() +
//         ")");
//        Logger.e(TAG, "Default Picture size (w,h) (" + tempPictureSize.width + "," + tempPictureSize.height +
//                        ")(" + sizePair.picture.getWidth() + "," + sizePair.picture.getHeight() +
//                        ")");
        ...
        int jpegQuality = CameraProfile.getJpegEncodingQualityParameter(
                requestedCameraId, CameraProfile.QUALITY_HIGH);
        ...
        if (value >= min && value <= max) {
            int vvv = parameters.getExposureCompensation();
            parameters.setExposureCompensation(value);
        } else {
            AccuraFaceMatchLog.loge(TAG, "invalid exposure range: " + value);
        }
...
//                Logger.e(TAG, "Camera auto focus is not supported on this device.");
            }
        }
    ...
    private void GetCameraResolution(Camera camera) {
        ...
        if (mParameters != null) {
            ...
            for (int i = 0; i < sizes.size(); i++) {
                ...
                float resolution1 = (result.width * result.height) / 1024000f;
                if (resolution1 > resolution) resolution = resolution1;
            }
        }
    }
    ...
    public void initializeFirstTime() {
        ...
        initializeScreenBrightness(activity.getWindow(), activity.getContentResolver());
        mFirstTimeInitialized = true;
    }

    private void initializeScreenBrightness(Window win, ContentResolver resolver) {
        // Override the brightness settings if it is automatic
...
//                matrix1.postRotate(displayOrientation);
//                Bitmap capturebitmap = Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(), matrix1, true);
//              }
//            };
    ...
    private void setRotation(Camera camera, Camera.Parameters parameters, int cameraId) {
        ...
        switch (rotation) {
            ...
            case Surface.ROTATION_270:
                ...
                break;
            default:
                AccuraFaceMatchLog.loge(TAG, "Bad rotation value: " + rotation);
        }
        ...
        this.displayOrientation = displayAngle;
...
//                            Logger.e(TAG, "Frame processing loop terminated.", e);
                            return;
                        }
                    }
...
//                    Logger.e(TAG, "Exception thrown from receiver.", t);
                } finally {
